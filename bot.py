...
####################################################
cl = LINE("EPeqCieX2tKbyXLaIKb1.5oFpoAaPat57wwbCTlcqGq.EMkEP2DlqH5sK9t7WLxSnEbtHOj5k1QpcwoQZLv4q6s=")
####################################################
    ...
    try:
        ...
        if op.type == 5:
            ...
            if settings["autoAdd"] == False:
                ...
                cl.sendMessage(op.param1, "安安！{} 感謝您加我為好友！半垢V4.0  Bot運行中(๑′ᴗ‵๑)！Çręätør:Yuan".format(str(cl.getContact(op.param1).displayName)))
        if op.type == 11:
        ...
        if op.type == 17:
            ...
            if settings["seeJoin"] == True:
                try:
                    ...
                    arrData = {'S':slen, 'E':elen, 'M':op.param2}
                    arr.append(arrData)
                    text += mention + "這裡是🧸蒂蒂Shop 👉🏻國外代購還有眾多商品可挑選🛒\n\n🎐每個月都有購買獎的抽獎活動不管買多少都可以抽獎\n\n🥰買魅雪系列產品一律免運優惠\n📣開放儲值金送購物金到五月止\n📣每樣商品都有結單日除非沒註明的我會在通知\n\n❣️基本上到貨時間2週到一個月以上不含假日為主（目前受疫情影響到貨速度會延遲一點點）\n蒂蒂都會追貨各位不用擔心哦\n到貨都會實放相簿並通知各位\n\n❣️群組一律匯款完成後訂貨\n除非要自取的可以貨到付款\n（匯款帳號在相簿內）\n\n📌如商品有斷貨會通知並退款\n蒂蒂的用心希望大家感受得到\n💞邀請朋友入群組享有$50購物金哦🙋🏻‍♀️請各位多多支持".format(str(group.name))
                    cl.sendMessage(op.param1,text, {'MENTION': str('{"MENTIONEES":' + json.dumps(arr) + '}')}, 0)
                except Exception as error:
                    print(error)
